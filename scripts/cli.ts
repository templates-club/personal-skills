...
function getExistingSubmodulePaths(): string[] {
  ...
  return Array.from(matches, (match: RegExpMatchArray) => match[1].trim())
}
...
async function initSubmodules(skipPrompt = false) {
  ...
  const expectedPaths = new Set(allProjects.map(proj => proj.path))
  ...
  const existingProjects = allProjects.filter(proj => submoduleExists(proj.path))
  const newProjects = allProjects.filter(proj => !submoduleExists(proj.path))
  ...
  if (existingProjects.length > 0)
    p.log.info(`Already initialized: ${existingProjects.map(proj => proj.name).join(', ')}`)
}
...
async function checkUpdates() {
  ...
  for (const [name, config] of Object.entries(vendors)) {
    ...
    const count = behind ? Number.parseInt(behind) : 0
    if (count > 0)
      updates.push({ name: `${name} (${Object.values(vendorConfig.skills).join(', ')})`, type: 'vendor', behind: count })
  }
...
async function cleanup(skipPrompt = false) {
  ...
  const existingSubmodulePaths = getExistingSubmodulePaths()
  const expectedSubmodulePaths = new Set(allProjects.map(proj => proj.path))
  ...
  if (extraSubmodules.length > 0) {
    ...
    for (const path of extraSubmodules)
      p.log.message(` - ${path}`)
    const shouldRemove = skipPrompt ? true : await p.confirm({ message: 'Remove these extra submodules?', initialValue: true })
    if (p.isCancel(shouldRemove)) { p.cancel('Cancelled'); return }
    if (shouldRemove) {
      hasChanges = true
      for (const submodulePath of extraSubmodules) {
        spinner.start(`Removing submodule: ${submodulePath}`)
  ...
  if (extraSkills.length > 0) {
    ...
    const shouldRemove = skipPrompt ? true : await p.confirm({ message: 'Remove these extra skills?', initialValue: true })
    if (p.isCancel(shouldRemove)) { p.cancel('Cancelled'); return }
    if (shouldRemove) {
      hasChanges = true
